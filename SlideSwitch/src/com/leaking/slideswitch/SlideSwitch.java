...
public class SlideSwitch extends View {
    ...
    private static final float OFFSET_NOT_SET = -1;
    private static final float DEFAULT_OFFSET_PERCENT = 0.55f;
    ...
    public SlideSwitch(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        offsetPercent = a.getFloat(R.styleable.slideswitch_offsetPercent, OFFSET_NOT_SET);
        a.recycle();
    ...
    public void initDrawingVal() {
        ...
        min_left = RIM_SIZE;
        if(offsetPercent == OFFSET_NOT_SET) {
            if (shape == SHAPE_RECT) {
                max_left = width / 2;
            } else {
            ...
            max_left = width - Math.round(width * offsetPercent);
        }
        if (isOpen) {
    ...
    protected void onDraw(Canvas canvas) {
        if (shape == SHAPE_RECT) {
            ...
            int maxCutoff;
            if(offsetPercent == OFFSET_NOT_SET){
                maxCutoff = getMeasuredWidth() / 2;
            }else{
                maxCutoff = Math.round(getMeasuredWidth() * offsetPercent);
            }
            frontRect.set(frontRect_left, RIM_SIZE, frontRect_left
                    + maxCutoff - RIM_SIZE, getMeasuredHeight()
                    - RIM_SIZE);
            ...
            int maxCutoff;
            if(offsetPercent == OFFSET_NOT_SET){
                maxCutoff = backRect.height() - 2 * RIM_SIZE;
            }else{
                maxCutoff = Math.round(getMeasuredWidth() * offsetPercent) - RIM_SIZE;
            }
            frontRect.set(frontRect_left, RIM_SIZE, frontRect_left + maxCutoff , backRect.height()
                    - RIM_SIZE);
